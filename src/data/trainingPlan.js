...
export const achievements = [
  { id: "starter", title: "First Run", detail: "Complete your first run.", unlockAt: 1 },
  { id: "first-week", title: "First Week", detail: "Finish a full week of training.", unlockAt: 'dynamic' },
  { id: "five", title: "5 Sessions", detail: "Complete 5 runs.", unlockAt: 5 },
  { id: "ten", title: "10 Sessions", detail: "Hit double digits.", unlockAt: 10 },
  { id: "halfway", title: "Halfway There", detail: "Complete 50% of your plan.", unlockAt: 'dynamic' },
  { id: "twenty-min", title: "20-Minute Run", detail: "Run 20 minutes continuously.", unlockAt: 'dynamic' },
  { id: "thirty-min", title: "30-Minute Run", detail: "Run 30 minutes continuously.", unlockAt: 'dynamic' },
  { id: "perfect-week", title: "Perfect Week", detail: "Nail all scheduled runs for the week without delay.", unlockAt: 'dynamic' },
  { id: "comeback", title: "Comeback", detail: "Log a run after a missed session or taking a recovery week.", unlockAt: 'dynamic' },
];

export function getUnlockedAchievements(appState, coreSessionsLength, streak) {
  const count = appState.progress.completedSessionIds.length;
  const longestRun = Math.max(0, ...appState.progress.sessionLogs.map(item => item.longestRun), 0);
  
  return achievements.filter(a => {
    switch(a.id) {
       case "starter": return count >= 1;
       case "first-week": return count >= 3 || streak >= 1; 
       case "five": return count >= 5;
       case "ten": return count >= 10;
       case "halfway": return count > 0 && count >= Math.floor(coreSessionsLength / 2);
       case "twenty-min": return longestRun >= 20;
       case "thirty-min": return longestRun >= 30;
       case "perfect-week": return streak >= 1;
       case "comeback": return appState.progress.sessionLogs.some(log => log.kind === "recovery" || log.kind === "repeat");
